...
**检查**：`<style id="theme-style">` 块必须包含有效的 CSS 主题。

**验证**：
```bash
grep -c "theme-style" index.html  # 应为 1
grep -A5 "theme-style" index.html  # 应显示 CSS 规则，不是空的或占位符
```

**修复**：将所选主题文件（如 `references/styles/minimalism.css`）的完整内容复制到 `<style id="theme-style">` 块中。

---

### GSAP 脚本标签

**检查**：必须存在 GSAP CDN 脚本，动画才能正常工作。

**验证**：
```bash
grep -c "gsap" index.html  # 应 >= 1
```
...
**检查**：如果需要编辑模式，必须包含 edit-mode.js 脚本。

**验证**：
```bash
grep -c "edit-mode.js" index.html  # 如果需要编辑模式，应为 1
```
...
**检查**：生成的 HTML 不能包含 `[必填]` 等占位符文本。

**验证**：
```bash
grep -c "\[必填\]" index.html  # 应为 0
```

**修复**：将 `<title>` 标签中的 `[必填] 替换为 PPT 标题 · Deck Title` 替换为实际的 PPT 标题。

---

### 无原始作者痕迹

**检查**：生成的 HTML 和所有参考文件不能包含原始作者的痕迹。

**验证**：检查 index.html、references/、SKILL.md、README.md 中是否包含已知的作者标识符。应无匹配结果。

**修复**：删除所有对原始作者的引用。如需要，使用通用描述替代。

<!-- FUTURE: Swiss Style support — the items below (0-S through 0-F) reference files and features not yet available in this project -->
...
**做法**:
- 原始参考 PPT 是 Swiss 主题的 golden source,但要以**实际页面用法**为准,不要只看未使用的 CSS helper
...
**做法**：
- **生成 PPT 前,必须先 `Read` 当前风格对应模板**:读 `assets/template.html`,确认 layouts 里用到的类都已定义
...
预检(生成前)
  ...
  <!-- FUTURE: Swiss Style — the items below apply when Swiss style support is available -->
