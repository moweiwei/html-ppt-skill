...
> **风格说明**:本文档用"风格 A"和"风格 B"作为两组配图提示词的简称。实际 skill 支持 7 种视觉风格(极简主义、液态玻璃、动态网格、扁平化、复古未来、手绘插画、暗黑模式)。风格 A 的提示词适用于大多数风格,风格 B(瑞士国际主义)是一套独立的、更严格的子规则。选择哪组提示词取决于用户选定的 deck 风格,而非本文档的编号。
...
> **Future**:本节引用的 `template-swiss.html` 和 `layouts-swiss.md` 尚未创建,属于规划中的模板文件。当前 7 种风格均使用 `assets/base.html` 作为基础模板。当瑞士国际主义风格模板落地时,本节提示词将与其配套使用。

当 deck 选择 Swiss Style 时,优先使用下面这组提示词。它们和 GPT-M 2.0 配套,目标是生成能直接放进原始登记版式的图片槽位,尤其是 S22 顶部横幅、S15/S16 多图网格。
...
下表为 7 种风格推荐搭配的图片类型,帮助快速选择提示词。

| 风格 | 推荐图片类型 | 说明 |
|------|-------------|------|
| **极简主义** | 类型 1(纪实照片)、类型 2(信息图)、类型 7(数据大字报) | 大量留白与克制配色,适合真实照片和干净的信息图 |
| **液态玻璃** | 类型 2(信息图)、类型 5(系统关系图)、类型 7(数据大字报) | 半透明层叠适合展示层级关系和数据高亮 |
| **动态网格** | 类型 2(信息图)、类型 6(截图再设计)、类型 7(数据大字报) | 卡片网格布局天然适配信息图和 UI 截图 |
| **扁平化** | 类型 3(流程图)、类型 4(对比图)、类型 5(系统关系图) | 高对比色块和无渐变风格适合结构化图表 |
| **复古未来** | 类型 4(对比图)、类型 6(截图再设计)、类型 7(数据大字报) | 霓虹和像素质感适合夸张的数据展示和 UI 重绘 |
| **手绘插画** | 类型 1(纪实照片)、类型 3(流程图)、类型 4(对比图) | 纸张纹理和宽松排版适合叙事性内容和流程讲解 |
| **暗黑模式** | 类型 2(信息图)、类型 5(系统关系图)、类型 7(数据大字报) | 深色底适合高对比信息图和数据可视化 |

> **提示**:上表为推荐组合,非硬性约束。所有 7 种图片类型均可用于任意风格,只需在提示词中微调色彩和质感描述以匹配目标风格。
